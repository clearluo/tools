...
func Mls() {
	...
	//C60000-62000
	//B33000-34000
	for no := 33000; no < 34000; no++ {
		bib := fmt.Sprintf("B%d", no)
		for _, name := range baijia {
			//fmt.Println(i, name)
			if sendPost(bib, name, no) {
				break
			}
		}
	}
...
func sendPost(bib string, name string, no int) bool {
	apiUrl := "https://results.runnar.com/score/getScoreUserScPagesv2"
	//resource := "/tpost"
	data := url.Values{}
	data.Set("bib", bib)
	data.Set("name", name)
	data.Set("pageNo", "1")
	data.Set("pageCount", "20")
	data.Set("eventId", "196")
	data.Set("projectId", "345")
	...
	if ret.ResultCode != "0000" {
		if ret.ResultCode != "7001" {
			fmt.Println("err:", ret.ResultCode, ret.ResultDesc)
		}
		return false
	}
